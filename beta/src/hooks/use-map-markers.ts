...
export default function useMapMarkers() {
  ...
  const coords: { lat: number; lng: number; id: string; shortcut: string; title: string }[] = [];
  ...
  if (categories.length > 0) {
    categories.forEach((category) => {
      ...
      coords.push(
        ...matches.map((entry) => {
          ...
          return { lat, lng, id: entry.entry_id, shortcut: entry.shortcut, title: entry.entry_title };
        })
      );
    });
    ...
    coords.push({ lat, lng, id: entry.entry_id, shortcut: entry.shortcut, title: entry.entry_title });
  }
